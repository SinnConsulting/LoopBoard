  ...
  async handleMessage(msg: any): Promise<void> {
    ...
    switch (msg?.type) {
      ...
      case 'attach': {
        ...
        // scoped to an already-open Description, answer, or note field (`field` set, keyed by
        // `reqId`) only stages the bytes here — the webview folds the returned link into that
        // field's own draft value and saves it through the normal field-patch path, so it lands
        // in the right place instead of always the Description (t-b149: note-scoped paste-attach).
        const taskId = String(msg.taskId ?? '');
        const filename = String(msg.filename ?? '');
        if (!taskId || !filename || typeof msg.dataBase64 !== 'string') return;
        const field = msg.field === 'description' || msg.field === 'answer' || msg.field === 'title' || msg.field === 'note' ? msg.field : undefined;
