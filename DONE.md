...
- [x] Add a per-task groom-model selector alongside the existing execution-model select on New/draft cards
  ...
  - question: ❓ Which grammar keyword for the field — `groom model:` (used throughout the groom; recommended), `groom:`, or `groomer:`? This fixes the parser case, writer line, and docs.
    - answer: groomer
  - feedback: ⚠️ ADDRESSED 2026-07-14 (fable): the "+ New Story" composer now has BOTH selectors inline — "Groom with" (sets `groomer:`) and "Model" (sets the execution `model:`) — so you pick them while writing the story; the draft is created with both fields already set. Landed on main @ c73a205 and the .vsix reinstalled — reload the VSCode window, click "+ New Story" to see them.
...
- [x] Render task descriptions as formatted markdown, editable as raw text on focus
  ...
  - feedback: ⚠️ FIXED 2026-07-12 (main @ f535924): clicking into a description then out without editing left the raw textarea showing instead of the rendered view — the blur handler set editingDesc=false but never called render(), so it relied on a file round-trip that never comes when the value is unchanged (sendPatch no-ops). Blur now calls render() like the Escape/click paths. Reload the Extension Development Host to pick it up.
...
- [x] Track phase via a task field to avoid rewriting whole sections on every move
  ...
  - feedback: ⚠️ LGTM. That worked TY!
