...
## Goals
- <groomer-owned; one verifiable outcome per bullet — review judges `## Delivered` against these>
...
14. New/DRAFT grooming is routed by `groomer:` (absent = default model; `none` = ON HOLD — the
    ...
    your report. Subagent expands the story into the task file's `## Problem` (short, factual),
    `## Description`, and `## Goals` (bullet list, the yardstick review judges `## Delivered`
    ...
    from the index (mirrors Rule 16). A still-present filled answer = not yet incorporated.
