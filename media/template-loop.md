...
7. Never commit to `main`: all work happens on a `task/**` branch off latest `main`. A PR is
   OPTIONAL — open one only if the user requested it. Before setting `phase: review`, write
   `## Delivered` and record the delivery in the task file's `link:`: the PR URL when a PR
...
12. No stranded work: every change from working a task is committed and pushed to its `task/**`
    ...
    uncommitted code in the checkout. No PR is required; record the delivery per Rule 7.
