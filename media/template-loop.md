...
7. Never commit to `main`: all work happens on a `task/**` branch off latest `main`. A PR is
   OPTIONAL — open one when review or a CI artifact comment is wanted. Before setting
   `phase: review`, record the delivery in the task file's `link:`: the PR URL when a PR
   exists, otherwise the `task/**` branch name. Never a commit sha; `link:` must be non-empty
   either way.
...
9. git worktrees forbidden (break pre-commit hooks assuming `.git` is a directory;
   ...
   commit and push from there; open a PR from it only if you want one (Rule 7).
...
12. No stranded work: every change from working a task is committed and pushed to its `task/**`
    branch before the session ends — never leave uncommitted code sitting in the checkout. No PR
    is required; record the delivery in the task file per Rule 7.
...
14. New/DRAFT grooming is routed by `groomer:` (absent = default model): the loop whose model
    ...
    explicitly asks for deep reasoning. Subagent expands the story into the task file's
