...
* **The line budget is the backstop.** `test/template-budget.test.js` asserts a hard maximum line
  ...
  commit. It binds editors that never see this rule, including a delegated implementer subagent,
  which cannot invoke skills unless one is explicitly handed to it. CI runs it too: `media/**` is
  in `.github/workflows/build.yml`'s `paths:` allowlist. Raising the budget requires a deliberate
  edit to that test and is a decision, not a formality: compress first, and only then raise it.
...
* **Untouchable, verbatim:** the six `<!-- loopboard:sync:<id>:begin/end -->` marker comments (in
  ...
  as single-quoted shell argv); Rule numbering as the input has it (1–16 today); the
  task-index-format and task-file-format
